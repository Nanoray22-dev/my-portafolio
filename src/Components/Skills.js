...
const Skills = () => {
  return (
    <div>
        ...
        <img src={skill} className="w-[70px] hover:animate-bounce " about='' alt=''/>
        <h1 className='text-[40px] font-bold'>{AppText.Skills}<span className='text-green-600'>{AppText.Experties}</span></h1>
        </div>
        <div className='flex flex-col md:flex-row  justify-evenly px-8 md:px-0 mt-8 '>
          <div className='grid grid-cols-4 md:grid-cols-4 gap-8  items-center'>
            {skillsList.map((item, index)=>(
                <div className='w-[60px] transition ease-in-out delay-100 hover:scale-125 bg-green-50
                 p-3 rounded-full' >
                    <img src={item.icon} 
                    className="w-[60px]" alt=''/>
                </div>
                ...
                <div className='flex mt-5 flex-col justify-end  md:mt-0 lg:mt-0'>
                    {workDetail.map((item,index)=>(
                        <div className='flex flex-row mb-6'>
                            ...
                            <div> 
                               <h3 className='font-thin text-[15px] text-gray-400'>{item.academy}</h3>
